...
const ContractVersion = "connector-contract-v16"
...
// descriptor. Integration retains reference resolution and persistence ownership.
...
// CallRequest is the type-erased Integration-to-provider envelope. Payload is
// decoded by a BoundOperation before project provider code is invoked. Secrets
// contains Integration-resolved material only for fields declared by the selected
...
// Integration never infers quota or billing state from a generic failed call.
...
// Gateway is implemented by the Integration execution boundary. Generated clients may call it only
// through the typed helpers in this package.
type Gateway interface {
	Call(context.Context, CallRequest) (CallResult, error)
}

// Adapter is the single Integration-facing project Connector boundary. One
...
// ConfigValidator is optional. Integration supplies the normalized connection and
...
// ConnectionTester is optional. Integration discovers it by type assertion and
...
// WebhookVerifier is optional. Integration owns the HTTP route, request limits,
